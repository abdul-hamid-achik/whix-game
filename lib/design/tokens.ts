import { z } from 'zod';

// Color token schema
const ColorTokenSchema = z.object({
  50: z.string(),
  100: z.string(),
  200: z.string(),
  300: z.string(),
  400: z.string(),
  500: z.string(),
  600: z.string(),
  700: z.string(),
  800: z.string(),
  900: z.string(),
  950: z.string(),
});

// Typography token type is defined inline in DesignTokensSchema

// Spacing token schema
const SpacingTokenSchema = z.record(z.string(), z.string());

// Animation token schema
const AnimationTokenSchema = z.object({
  duration: z.object({
    instant: z.string(),
    fast: z.string(),
    normal: z.string(),
    slow: z.string(),
    verySlow: z.string(),
  }),
  easing: z.object({
    linear: z.string(),
    ease: z.string(),
    easeIn: z.string(),
    easeOut: z.string(),
    easeInOut: z.string(),
    bounce: z.string(),
    cyberpunk: z.string(), // Custom easing for glitch effects
  }),
});

// Shadow token schema
const ShadowTokenSchema = z.object({
  sm: z.string(),
  md: z.string(),
  lg: z.string(),
  xl: z.string(),
  '2xl': z.string(),
  inner: z.string(),
  none: z.string(),
  // Cyberpunk-specific shadows
  neon: z.string(),
  glitch: z.string(),
});

// Border radius token schema
const BorderRadiusTokenSchema = z.object({
  none: z.string(),
  sm: z.string(),
  md: z.string(),
  lg: z.string(),
  xl: z.string(),
  '2xl': z.string(),
  '3xl': z.string(),
  full: z.string(),
});

// Complete design tokens schema
export const DesignTokensSchema = z.object({
  colors: z.object({
    // Brand colors
    primary: ColorTokenSchema,
    secondary: ColorTokenSchema,
    accent: ColorTokenSchema,
    
    // Neutral colors
    gray: ColorTokenSchema,
    
    // Semantic colors
    success: ColorTokenSchema,
    warning: ColorTokenSchema,
    error: ColorTokenSchema,
    info: ColorTokenSchema,
    
    // Game-specific colors
    cyber: ColorTokenSchema,     // Cyan/blue cyberpunk colors
    neon: ColorTokenSchema,      // Bright neon colors
    corporate: ColorTokenSchema, // WHIX corporate colors
    neural: ColorTokenSchema,    // Neural interface colors
    
    // Special colors
    humanity: z.object({
      high: z.string(),
      medium: z.string(),
      low: z.string(),
    }),
    rarity: z.object({
      common: z.string(),
      uncommon: z.string(),
      rare: z.string(),
      epic: z.string(),
      legendary: z.string(),
    }),
  }),
  
  typography: z.object({
    // Font families
    fontFamily: z.object({
      sans: z.string(),
      mono: z.string(),
      display: z.string(),
      cyber: z.string(), // Cyberpunk-style font
    }),
    
    // Font sizes
    fontSize: z.object({
      xs: z.string(),
      sm: z.string(),
      base: z.string(),
      lg: z.string(),
      xl: z.string(),
      '2xl': z.string(),
      '3xl': z.string(),
      '4xl': z.string(),
      '5xl': z.string(),
      '6xl': z.string(),
      '7xl': z.string(),
      '8xl': z.string(),
      '9xl': z.string(),
    }),
    
    // Font weights
    fontWeight: z.object({
      thin: z.string(),
      extralight: z.string(),
      light: z.string(),
      normal: z.string(),
      medium: z.string(),
      semibold: z.string(),
      bold: z.string(),
      extrabold: z.string(),
      black: z.string(),
    }),
    
    // Line heights
    lineHeight: z.object({
      none: z.string(),
      tight: z.string(),
      snug: z.string(),
      normal: z.string(),
      relaxed: z.string(),
      loose: z.string(),
    }),
    
    // Letter spacing
    letterSpacing: z.object({
      tighter: z.string(),
      tight: z.string(),
      normal: z.string(),
      wide: z.string(),
      wider: z.string(),
      widest: z.string(),
    }),
  }),
  
  spacing: SpacingTokenSchema,
  
  animation: AnimationTokenSchema,
  
  shadows: ShadowTokenSchema,
  
  borderRadius: BorderRadiusTokenSchema,
  
  // Z-index layers
  zIndex: z.object({
    base: z.number(),
    dropdown: z.number(),
    sticky: z.number(),
    fixed: z.number(),
    modalBackdrop: z.number(),
    modal: z.number(),
    popover: z.number(),
    tooltip: z.number(),
    notification: z.number(),
  }),
  
  // Breakpoints for responsive design
  breakpoints: z.object({
    sm: z.string(),
    md: z.string(),
    lg: z.string(),
    xl: z.string(),
    '2xl': z.string(),
  }),
});

export type DesignTokens = z.infer<typeof DesignTokensSchema>;

// Default design tokens
export const defaultTokens: DesignTokens = {
  colors: {
    primary: {
      50: '#f0f9ff',
      100: '#e0f2fe',
      200: '#bae6fd',
      300: '#7dd3fc',
      400: '#38bdf8',
      500: '#0ea5e9',
      600: '#0284c7',
      700: '#0369a1',
      800: '#075985',
      900: '#0c4a6e',
      950: '#082f49',
    },
    secondary: {
      50: '#faf5ff',
      100: '#f3e8ff',
      200: '#e9d5ff',
      300: '#d8b4fe',
      400: '#c084fc',
      500: '#a855f7',
      600: '#9333ea',
      700: '#7e22ce',
      800: '#6b21a8',
      900: '#581c87',
      950: '#3b0764',
    },
    accent: {
      50: '#fefce8',
      100: '#fef9c3',
      200: '#fef08a',
      300: '#fde047',
      400: '#facc15',
      500: '#eab308',
      600: '#ca8a04',
      700: '#a16207',
      800: '#854d0e',
      900: '#713f12',
      950: '#422006',
    },
    gray: {
      50: '#f9fafb',
      100: '#f3f4f6',
      200: '#e5e7eb',
      300: '#d1d5db',
      400: '#9ca3af',
      500: '#6b7280',
      600: '#4b5563',
      700: '#374151',
      800: '#1f2937',
      900: '#111827',
      950: '#030712',
    },
    success: {
      50: '#f0fdf4',
      100: '#dcfce7',
      200: '#bbf7d0',
      300: '#86efac',
      400: '#4ade80',
      500: '#22c55e',
      600: '#16a34a',
      700: '#15803d',
      800: '#166534',
      900: '#14532d',
      950: '#052e16',
    },
    warning: {
      50: '#fffbeb',
      100: '#fef3c7',
      200: '#fde68a',
      300: '#fcd34d',
      400: '#fbbf24',
      500: '#f59e0b',
      600: '#d97706',
      700: '#b45309',
      800: '#92400e',
      900: '#78350f',
      950: '#451a03',
    },
    error: {
      50: '#fef2f2',
      100: '#fee2e2',
      200: '#fecaca',
      300: '#fca5a5',
      400: '#f87171',
      500: '#ef4444',
      600: '#dc2626',
      700: '#b91c1c',
      800: '#991b1b',
      900: '#7f1d1d',
      950: '#450a0a',
    },
    info: {
      50: '#eff6ff',
      100: '#dbeafe',
      200: '#bfdbfe',
      300: '#93c5fd',
      400: '#60a5fa',
      500: '#3b82f6',
      600: '#2563eb',
      700: '#1d4ed8',
      800: '#1e40af',
      900: '#1e3a8a',
      950: '#172554',
    },
    cyber: {
      50: '#ecfeff',
      100: '#cffafe',
      200: '#a5f3fc',
      300: '#67e8f9',
      400: '#22d3ee',
      500: '#06b6d4',
      600: '#0891b2',
      700: '#0e7490',
      800: '#155e75',
      900: '#164e63',
      950: '#083344',
    },
    neon: {
      50: '#fdf4ff',
      100: '#fae8ff',
      200: '#f5d0fe',
      300: '#f0abfc',
      400: '#e879f9',
      500: '#d946ef',
      600: '#c026d3',
      700: '#a21caf',
      800: '#86198f',
      900: '#701a75',
      950: '#4a044e',
    },
    corporate: {
      50: '#f8fafc',
      100: '#f1f5f9',
      200: '#e2e8f0',
      300: '#cbd5e1',
      400: '#94a3b8',
      500: '#64748b',
      600: '#475569',
      700: '#334155',
      800: '#1e293b',
      900: '#0f172a',
      950: '#020617',
    },
    neural: {
      50: '#f0fdfa',
      100: '#ccfbf1',
      200: '#99f6e4',
      300: '#5eead4',
      400: '#2dd4bf',
      500: '#14b8a6',
      600: '#0d9488',
      700: '#0f766e',
      800: '#115e59',
      900: '#134e4a',
      950: '#042f2e',
    },
    humanity: {
      high: '#22c55e',
      medium: '#f59e0b',
      low: '#ef4444',
    },
    rarity: {
      common: '#9ca3af',
      uncommon: '#22c55e',
      rare: '#3b82f6',
      epic: '#a855f7',
      legendary: '#f59e0b',
    },
  },
  
  typography: {
    fontFamily: {
      sans: 'Inter, system-ui, -apple-system, sans-serif',
      mono: 'JetBrains Mono, Consolas, monospace',
      display: 'Orbitron, sans-serif',
      cyber: 'Rajdhani, sans-serif',
    },
    fontSize: {
      xs: '0.75rem',
      sm: '0.875rem',
      base: '1rem',
      lg: '1.125rem',
      xl: '1.25rem',
      '2xl': '1.5rem',
      '3xl': '1.875rem',
      '4xl': '2.25rem',
      '5xl': '3rem',
      '6xl': '3.75rem',
      '7xl': '4.5rem',
      '8xl': '6rem',
      '9xl': '8rem',
    },
    fontWeight: {
      thin: '100',
      extralight: '200',
      light: '300',
      normal: '400',
      medium: '500',
      semibold: '600',
      bold: '700',
      extrabold: '800',
      black: '900',
    },
    lineHeight: {
      none: '1',
      tight: '1.25',
      snug: '1.375',
      normal: '1.5',
      relaxed: '1.625',
      loose: '2',
    },
    letterSpacing: {
      tighter: '-0.05em',
      tight: '-0.025em',
      normal: '0em',
      wide: '0.025em',
      wider: '0.05em',
      widest: '0.1em',
    },
  },
  
  spacing: {
    px: '1px',
    '0': '0px',
    '0.5': '0.125rem',
    '1': '0.25rem',
    '1.5': '0.375rem',
    '2': '0.5rem',
    '2.5': '0.625rem',
    '3': '0.75rem',
    '3.5': '0.875rem',
    '4': '1rem',
    '5': '1.25rem',
    '6': '1.5rem',
    '7': '1.75rem',
    '8': '2rem',
    '9': '2.25rem',
    '10': '2.5rem',
    '11': '2.75rem',
    '12': '3rem',
    '14': '3.5rem',
    '16': '4rem',
    '20': '5rem',
    '24': '6rem',
    '28': '7rem',
    '32': '8rem',
    '36': '9rem',
    '40': '10rem',
    '44': '11rem',
    '48': '12rem',
    '52': '13rem',
    '56': '14rem',
    '60': '15rem',
    '64': '16rem',
    '72': '18rem',
    '80': '20rem',
    '96': '24rem',
  },
  
  animation: {
    duration: {
      instant: '0ms',
      fast: '150ms',
      normal: '300ms',
      slow: '500ms',
      verySlow: '1000ms',
    },
    easing: {
      linear: 'linear',
      ease: 'ease',
      easeIn: 'ease-in',
      easeOut: 'ease-out',
      easeInOut: 'ease-in-out',
      bounce: 'cubic-bezier(0.68, -0.55, 0.265, 1.55)',
      cyberpunk: 'cubic-bezier(0.23, 1, 0.32, 1)',
    },
  },
  
  shadows: {
    sm: '0 1px 2px 0 rgb(0 0 0 / 0.05)',
    md: '0 4px 6px -1px rgb(0 0 0 / 0.1), 0 2px 4px -2px rgb(0 0 0 / 0.1)',
    lg: '0 10px 15px -3px rgb(0 0 0 / 0.1), 0 4px 6px -4px rgb(0 0 0 / 0.1)',
    xl: '0 20px 25px -5px rgb(0 0 0 / 0.1), 0 8px 10px -6px rgb(0 0 0 / 0.1)',
    '2xl': '0 25px 50px -12px rgb(0 0 0 / 0.25)',
    inner: 'inset 0 2px 4px 0 rgb(0 0 0 / 0.05)',
    none: 'none',
    // Cyberpunk-specific shadows
    neon: '0 0 20px rgba(6, 182, 212, 0.5), 0 0 40px rgba(6, 182, 212, 0.3)',
    glitch: '2px 2px 0 rgba(255, 0, 255, 0.5), -2px -2px 0 rgba(0, 255, 255, 0.5)',
  },
  
  borderRadius: {
    none: '0px',
    sm: '0.125rem',
    md: '0.375rem',
    lg: '0.5rem',
    xl: '0.75rem',
    '2xl': '1rem',
    '3xl': '1.5rem',
    full: '9999px',
  },
  
  zIndex: {
    base: 0,
    dropdown: 10,
    sticky: 20,
    fixed: 30,
    modalBackdrop: 40,
    modal: 50,
    popover: 60,
    tooltip: 70,
    notification: 80,
  },
  
  breakpoints: {
    sm: '640px',
    md: '768px',
    lg: '1024px',
    xl: '1280px',
    '2xl': '1536px',
  },
};

// Helper function to get CSS variables from tokens
export function generateCSSVariables(tokens: DesignTokens): string {
  const cssVars: string[] = [];
  
  // Colors
  Object.entries(tokens.colors).forEach(([colorName, colorValue]) => {
    if (typeof colorValue === 'object' && !Array.isArray(colorValue)) {
      Object.entries(colorValue).forEach(([shade, value]) => {
        cssVars.push(`--color-${colorName}-${shade}: ${value};`);
      });
    }
  });
  
  // Typography
  Object.entries(tokens.typography.fontFamily).forEach(([name, value]) => {
    cssVars.push(`--font-${name}: ${value};`);
  });
  
  Object.entries(tokens.typography.fontSize).forEach(([size, value]) => {
    cssVars.push(`--text-${size}: ${value};`);
  });
  
  // Spacing
  Object.entries(tokens.spacing).forEach(([size, value]) => {
    cssVars.push(`--spacing-${size}: ${value};`);
  });
  
  // Animation
  Object.entries(tokens.animation.duration).forEach(([name, value]) => {
    cssVars.push(`--duration-${name}: ${value};`);
  });
  
  Object.entries(tokens.animation.easing).forEach(([name, value]) => {
    cssVars.push(`--easing-${name}: ${value};`);
  });
  
  // Shadows
  Object.entries(tokens.shadows).forEach(([name, value]) => {
    cssVars.push(`--shadow-${name}: ${value};`);
  });
  
  // Border radius
  Object.entries(tokens.borderRadius).forEach(([name, value]) => {
    cssVars.push(`--radius-${name}: ${value};`);
  });
  
  return `:root {\n  ${cssVars.join('\n  ')}\n}`;
}

// Export tokens for use in components
export const tokens = defaultTokens;