import { ContentPartnerAdapter, ContentPartner } from './content-partner-adapter';
...
/**
 * Content-based gacha system for recruiting actual characters
 */
export class ContentGachaSystem {
  ...
  static async pullGacha(
    ...
    ownedCharacters: string[], // contentIds of owned characters
    storyProgress: string[] = [],
    playerLevel: number = 1
  ): Promise<GachaPull> {
    const { characters: _characters, rarityWeights } = await ContentPartnerAdapter.getGachaPool();
    
    // Get available characters (not owned, unlocked)
    const availableCharacters = await ContentPartnerAdapter.getAvailableForRecruitment(
      storyProgress,
      ownedCharacters,
      playerLevel
    );
  ...
  static async getFeaturedCharacters(): Promise<ContentPartner[]> {
    const allCharacters = await ContentPartnerAdapter.loadAllContentCharacters();
    
    // Feature main story characters and unique characters
  ...
  /**
   * Simulate guaranteed rare+ in 10-pull
   */
  static ensureGuaranteedRare(results: GachaResult[]): void {
    const hasRareOrBetter = results.some(result => 
      ['rare', 'epic', 'legendary'].includes(result.partner.rarity)
    );
    
    if (!hasRareOrBetter && results.length > 0) {
      // Upgrade the last result to rare
      const lastResult = results[results.length - 1];
      if (lastResult.partner.rarity === 'common') {
        // Create a new rare version (simplified approach)
        lastResult.partner.rarity = 'rare';
        lastResult.partner.stats = Object.entries(lastResult.partner.stats).reduce(
          (stats, [key, value]) => {
            stats[key as keyof typeof stats] = Math.round(value * 1.2);
            return stats;
          }, 
          {} as typeof lastResult.partner.stats
        );
      }
    }
  }
